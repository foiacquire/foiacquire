...
CREATE TABLE archive_snapshots (
    ...
    captured_at TEXT NOT NULL,          -- When archive captured it (ISO8601)
    discovered_at TEXT NOT NULL,        -- When we found this snapshot
    http_status INTEGER,                -- Status code from archive metadata
    mimetype TEXT,                       -- MIME type from archive metadata
    content_length INTEGER,             -- File size from archive metadata
    digest TEXT,                        -- Archive's content hash (e.g., Wayback SHA-1)
    metadata TEXT NOT NULL DEFAULT '{}' -- Service-specific extras (JSON)
);
...
CREATE TABLE archive_checks (
    ...
    checked_at TEXT NOT NULL,           -- When we performed the check
...
-- archive_snapshot_id: set if this version was recovered FROM an archive
-- earliest_archived_at: oldest verified snapshot with same content hash
