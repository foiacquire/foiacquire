-- Initial schema for foiacquire PostgreSQL (v13)
...
INSERT INTO file_categories (id, description, doc_count) VALUES
...
-- PostgreSQL uses functions for trigger logic
CREATE OR REPLACE FUNCTION update_document_count_insert()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION update_document_count_delete()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION update_category_count_insert()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION update_category_count_delete()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION update_category_count_update()
RETURNS TRIGGER AS $$
BEGIN
...
DROP TRIGGER IF EXISTS tr_documents_insert ON documents;
DROP TRIGGER IF EXISTS tr_documents_delete ON documents;
DROP TRIGGER IF EXISTS tr_category_count_insert ON documents;
DROP TRIGGER IF EXISTS tr_category_count_delete ON documents;
DROP TRIGGER IF EXISTS tr_category_count_update ON documents;

CREATE TRIGGER tr_documents_insert
AFTER INSERT ON documents
FOR EACH ROW EXECUTE FUNCTION update_document_count_insert();

CREATE TRIGGER tr_documents_delete
AFTER DELETE ON documents
FOR EACH ROW EXECUTE FUNCTION update_document_count_delete();

CREATE TRIGGER tr_category_count_insert
AFTER INSERT ON documents
FOR EACH ROW EXECUTE FUNCTION update_category_count_insert();

CREATE TRIGGER tr_category_count_delete
AFTER DELETE ON documents
FOR EACH ROW EXECUTE FUNCTION update_category_count_delete();

CREATE TRIGGER tr_category_count_update
AFTER UPDATE OF category_id ON documents
FOR EACH ROW EXECUTE FUNCTION update_category_count_update();

-- Store schema version
INSERT INTO storage_meta (key, value) VALUES ('format_version', '13')
